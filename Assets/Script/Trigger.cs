...
{
    public GameObject[] platform = new GameObject[0];
    ...
    public void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Pusher") || collision.CompareTag("Puller"))
        {
            ...
            for (int i = 0; i < platform.Length; i++)
            {
                ...
                platform[i].GetComponent<SpriteRenderer>().color = new Color(1, 1, 1, 0.02f);
            }

            trigger.GetComponent<CircleCollider2D>().enabled = false;

            trigger.GetComponent<SpriteRenderer>().color = new Color(1, 1, 1, 0.02f);
            timeBox = Time.time;
        }
    }
    void Update()
